...
class Project {
    ...
    public function save(PDO $conn, $usersId) {
    ...
    public function update(PDO $conn, $id) {
        $stmt = $conn->prepare("UPDATE projecten SET naam = ?, beschrijving = ?, status = ?, project_datum = ? WHERE id = ?");
        return $stmt->execute([
            $this->title,
            $this->description,
            $this->category,
    ...
    public static function getByUsersId(PDO $conn, $user_id) {
        $stmt = $conn->prepare(
    ...
    public static function delete(PDO $conn, $id, $usersId) {
        $stmt = $conn->prepare("DELETE FROM projecten WHERE id = ? AND user_id = ?");
        return $stmt->execute([$id, $usersId]);
    }
}
?>